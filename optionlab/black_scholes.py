...
def getoptionprice(optype,s0,x,r,time2maturity,d1,d2,y=0.0):
    ...
    if optype=="call":		
        return round(s*stats.norm.cdf(d1)-x*exp(-r*time2maturity)*stats.norm.cdf(d2),2)
    elif optype=="put":
        return round(x*exp(-r*time2maturity)*stats.norm.cdf(-d2)-s*stats.norm.cdf(-d1),2)
    else:
...
def gettheta(optype,s0,x,r,vol,time2maturity,d1,d2,y=0.0):
    ...
    if optype=="call":
        return -(s*vol*cdf_d1_prime/(2.0*sqrt(time2maturity))+r*x*exp(-r*time2maturity)*stats.norm.cdf(d2)-
...
def get_d1_d2(s0,x,r,vol,time2maturity,y=0.0):
    ...
    '''           
    d1=(log(s0/x)+(r-y+vol*vol/2.0)*time2maturity)/(vol*sqrt(time2maturity))
